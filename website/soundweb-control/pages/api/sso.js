...
export default withSessionRoute(
  async function ssoRoute(req, res) {
    ...
    const challenge = (req && req.body && req.body.challenge) ? String(req.body.challenge) : undefined;
    ...
    if (!db_user) {
      res.send({
        status: "redirect",
        url: urlBuilder(sso_login_url, {callback: JSON.stringify(callback)})
      });
      return;
    }
    ...
    if (sso_app_id) {
      ...
      if (ssoAppDatabase.find((a) => (a.id === sso_app_id)) === undefined) {
        if (db_user.admin) {
          ...
          res.send({
            status: "redirect",
            url: urlBuilder(sso_access_denied_url, {callback: JSON.stringify(callback)})
          });
        } else {
          res.send({
            status: "redirect",
            url: urlBuilder(sso_access_denied_url, {new_app_id: 1, callback: JSON.stringify(callback)})
          });
        }
        return;
    ...
    const logout_url = urlBuilder(sso_logout_url, {callback: JSON.stringify(callback)});
